...
"
SRC_URI_ARM64="
        ${BASE_SRC_URI}/aarch64/LibreOffice_${PV}_Linux_aarch64_deb.tar.gz
"
...
handle_lang() {
	...
	for lang in ${LANGUAGES_HELP[@]}; do
		SRC_URI+=" l10n_${lang%:*}? (
			offlinehelp? (
				amd64? ( ${BASE_SRC_URI}/x86_64/LibreOffice_${PV}_Linux_x86-64_deb_helppack_${lang#*:}.tar.gz )
				arm64? ( ${BASE_SRC_URI}/aarch64/LibreOffice_${PV}_Linux_aarch64_deb_helppack_${lang#*:}.tar.gz )
			)
		)"
	done
	for lang in ${LANGUAGES[@]}; do
		if [[ ${lang%:*} != en ]]; then
			SRC_URI+=" l10n_${lang%:*}? (
				amd64? ( ${BASE_SRC_URI}/x86_64/LibreOffice_${PV}_Linux_x86-64_deb_langpack_${lang#*:}.tar.gz )
				arm64? ( ${BASE_SRC_URI}/aarch64/LibreOffice_${PV}_Linux_aarch64_deb_langpack_${lang#*:}.tar.gz )
			)"
		fi
		IUSE+=" l10n_${lang%:*}"
	done
...
src_unpack() {
	...
	rm "${WORKDIR}"/${BINPKG_BASE}/DEBS/libobasis${PV%*.*}-firebird*_*.deb || die
	...
	rm "${WORKDIR}"/${BINPKG_BASE}/DEBS/libobasis${PV%*.*}-kde-integration*_*.deb || die

	# Bundled Python is used (3.10 as of 25.2.0), so no need for system dependency.
	if ! use python ; then
		rm "${WORKDIR}"/${BINPKG_BASE}/DEBS/libobasis${PV%*.*}-python-script-provider*_*.deb || die
	fi
